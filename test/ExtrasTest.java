...
public class ExtrasTest {
	...
	public void cancelationTest() {
		...
		try {
			...
			assertTrue("Expected 7 messages processed found scheduler.getProcessedMessages()-->"
					+ scheduler.getProcessedMessages(), scheduler.getProcessedMessages() == 7);
	...
	public void terminationTest() {
		try {
			...
			assertTrue("Invalid number of Messages registered-->" + queue.getMessageCount(),
					queue.getMessageCount() == 0);
			assertTrue("Expected 8 messages processed found scheduler.getProcessedMessages()-->"
					+ scheduler.getProcessedMessages(), scheduler.getProcessedMessages() == 8);
	...
	public void alternativeMessagePriorizationTest() {
		...
		try {
			final ResourceScheduler scheduler = new ResourceScheduler("scheduler.queue.EvenGroupIdFirstQueueManager");
			final AbstractQueueManager queue = scheduler.getQueue();
			...
			assertTrue("Invalid number of Messages registered-->" + queue.getMessageCount(),
					queue.getMessageCount() == 0);
			assertTrue("Expected 12 messages processed found scheduler.getProcessedMessages()-->"
					+ scheduler.getProcessedMessages(), scheduler.getProcessedMessages() == 12);
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (i > 2) {
					final int groupId = list.get(i).getMsg().getGroupId();
