	...
	public void baseExampleTest() {
		...
		try {
			...
			assertTrue("Expected Gateway 0 and Message (1,2) found -->" + order,
					order.getGatewayId() == 0 && order.getMsg().getId() == 1);

			order = (ProcessingOrder) list.get(1);
			assertTrue("Expected Gateway 0 and Message (3,2) found -->" + order,
					order.getGatewayId() == 0 && order.getMsg().getId() == 3);

			order = (ProcessingOrder) list.get(2);
			assertTrue("Expected Gateway 0 and Message (2,1) found -->" + order,
					order.getGatewayId() == 0 && order.getMsg().getId() == 2);

			order = (ProcessingOrder) list.get(3);
			assertTrue("Expected Gateway 0 and Message (4,3) found -->" + order,
					order.getGatewayId() == 0 && order.getMsg().getId() == 4);
